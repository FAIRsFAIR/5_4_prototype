...
class catalog_harvester
{
    ...
    function __construct($catalog_url)
    {
        ...
        $this->catalog_url = rtrim($this->catalog_url,'/');
        if(strpos($this->catalog_url, 'http') === 0){
            ...
            $ejson = $this->get_embedded_jsonld();
        ...
        if(is_string($ginput) | is_a($ginput,'\EasyRdf\Graph')){
            ...
            try{
                ...
                if(is_a($jg,'\EasyRdf\Graph')) {
                    ...
                    #check for esip type
                    $is_esip_catalog = false;
                    $offer_catalogs = $jg->all($guri, 'schema:hasOfferCatalog/schema:itemListElement');
                    foreach($offer_catalogs as $oc){
                        if(strpos((string)$oc->type(), 'DataCatalog')!==false){
                            $is_esip_catalog = true;
                        }
                    }
                    if($is_esip_catalog === true){
                        $catalogs[] = $jg->resource($guri);
                        $this->logging[$guri][]= array('success'=>'Found ESIP style catalogue info (schema:Service, schema:ResearchProject) in '.strtoupper($format).' graph');
                    ...
                    foreach ($catalogs as $catalog) {
                        ...
                        foreach ($apis as $api) {
                            ...
                            $add_type = strtolower((string)$api->get('schema:additionalType|rdfs:seeAlso|schema:serviceType'));
                            if (!isset($api_url))
                                $api_url = (string)$api;
                            ...
                            if(strpos($add_type, 'metadata')){
                                $ctype = 'metadata';
                                $metadata['metadata'] = (string)$api_type;
                            }
                            if(strpos($add_type, 'identifier')){
                                $ctype = 'identifier';
                                $metadata['pid'] = (string)$api_type;
                            }
                            if($ctype =='api'){
                        ...
                        foreach ($compliances as $compliance) {
                            ...
                            if (strpos($ctype, 'standard') !== false) {
                                ...
                                } elseif (stripos($atype, 'identifier') !== false){
                                    $ptype = 'identifier';
                                    $metadata['pid'] = $curl;
                                }
                                ...
                                }elseif(preg_match('/accrual|submission|deposit(ion)?/', $curl)){
                                    if(preg_match('/terms?|policy|conditions?/',$curl)){
                                        $ptype = 'termsofdeposit';
                                        $metadata['termsofdeposit'] = $curl;
                                    }
                                }
